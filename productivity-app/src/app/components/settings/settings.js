...
function createTopScale() {
  ...
  middlePoint.textContent = "First cycle: " + minToHours(getFullCycle()/2);
...
function createBottomScale (){
  ...
  for (var i=30 ; i<getFullCycle();i+=30) {
    ...
    bottomScaleItem.textContent = minToHours(i);
...
function getFullCycle() {
  return (settingsItems[0].value + settingsItems[2].value) * settingsItems[1].value - 2 * settingsItems[2].value + settingsItems[3].value;
}
